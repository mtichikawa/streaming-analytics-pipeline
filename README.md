...
| `dashboards/app.py` | Streamlit dashboard (Day 4). |
...
Each Spark job writes to a checkpoint directory (`checkpoints/ohlcv_5m`, `checkpoints/alerts`). On restart it resumes from the last committed offsets, and the Delta sink's transactional commits give effective exactly-once on the output. To wipe state and start fresh:
...
The producer, schema, and z-score tests are pure Python and run anywhere. The OHLCV aggregation/watermark tests need a real SparkSession (Java + PySpark) and are automatically **skip-gated** when Spark isn't available, so the suite stays green in lightweight environments.
